    ...
    if(_flow[index] >= '0' &&_flow[index] <= '9')
    {
        ...
        for(int i = index + 1; i < _flow.size(); )
        {
           ...
           if(std::isspace(_flow[i]))
           {
            ...
            {   
                    index = i + 1;
                    throw ErrorReporter("unexpected char after a number");
                    return false;
            }
        }
    }
...
void microsyntax::PrintAns()
{
    for (auto &p : Words)
    {
        std::cout<<typetable[static_cast<int>(p.type)]<<", "<<p.str<<std::endl;
    }
}
